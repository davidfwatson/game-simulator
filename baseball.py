...
class BaseballSimulator:
    ...
    def _determine_outcome_from_trajectory(self, ev, la):
        ...
        # Ground balls
        if la < 5:
            if ev > 110: return "Groundout" # Hard grounder, high DP chance
    ...
    def _resolve_steal_attempt(self, base_to_steal):
        ...
        if self.game_rng.random() < success_chance:
            ...
            # This action will be described by the converter.
    ...
    def _simulate_at_bat(self, batter, pitcher):
        ...
        description = {'k_type': k_type, 'verb': verb}
    ...
    def _handle_batted_ball_out(self, out_type, batter, context=None):
        ...
        if out_type in ['Groundout', 'Sacrifice Bunt', 'Lineout', 'Pop Out', 'Forceout', 'Grounded Into DP']:
            # SS and 3B get the most ground balls
            weights = [10 if p['position']['abbreviation'] == 'SS' else 8 if p['position']['abbreviation'] == '3B' else 6 for p in infielders]
            grounder_candidates = list(zip(infielders, weights)) + [(pitcher, 1)] + ([(catcher, 0.25)] if catcher else [])
        ...
        if out_type == 'Groundout':
            ...
            dp_rate = defensive_team_data['double_play_rate']
            ev = batted_ball_data.get('ev', 90)

            # Hard-hit grounders are more likely to be double plays
            if ev > 105:
                dp_rate *= 1.5
            elif ev < 85:
                dp_rate *= 0.5

            # Most common DP situation: grounder to SS or 3B
            if self.outs < 2 and self.bases[0] and fielder['position']['abbreviation'] in ['SS', '3B']:
                dp_rate *= 1.8 # Increase chance significantly

            if self.outs < 2 and self.bases[0] and self.game_rng.random() < dp_rate:
                ...
                if fielder['position']['abbreviation'] == 'SS' and sb and first_baseman:
                    notation = "6-4-3"
                    credits = [
                        {'player': {'id': fielder['id']}, 'position': fielder['position'], 'credit': 'assist'},
                        {'player': {'id': sb['id']}, 'position': sb['position'], 'credit': 'assist'},
                        {'player': {'id': first_baseman['id']}, 'position': first_baseman['position'], 'credit': 'putout'}
                    ]
                    ...
                    notation = f"{fielder['position']['code']}U"
                    credits = [
                        {'player': {'id': fielder['id']}, 'position': fielder['position'], 'credit': 'putout'},
                        {'player': {'id': fielder['id']}, 'position': fielder['position'], 'credit': 'putout'}
                    ]
            ...
            if is_force_play and self.outs < 2:
                ...
                if ss:
                    credits = [
                        {'player': {'id': fielder['id']}, 'position': fielder['position'], 'credit': 'assist'},
                        {'player': {'id': ss['id']}, 'position': ss['position'], 'credit': 'putout'}
                    ]
        ...
        if 'play_label' in locals():
            if fielder['position']['abbreviation'] == '1B' and first_baseman:
                notation = "3U"
                credits = [{'player': {'id': first_baseman['id']}, 'position': first_baseman['position'], 'credit': 'putout'}]
            elif first_baseman:
    ...
    def _simulate_half_inning(self):
        if self.generate_gameday:
            while len(self.gameday_data['liveData']['linescore']['innings']) < self.inning:
                inning_num = len(self.gameday_data['liveData']['linescore']['innings']) + 1
                self.gameday_data['liveData']['linescore']['innings'].append({'num': inning_num, 'home': {'runs': 0}, 'away': {'runs': 0}})
...
if __name__ == "__main__":
    ...
    # The simulator now always runs in 'gameday' mode internally
    game = BaseballSimulator(
        TEAMS["BAY_BOMBERS"],
        TEAMS["PC_PILOTS"],
        verbose_phrasing=not args.terse,
        use_bracketed_ui=args.bracketed_ui,
        commentary_style='gameday', # This ensures gameday_data is generated
    ...
    # The new converter handles pbp/statcast generation
    from pbp_converter import GamedayConverter

    pbp_output = None
    if args.commentary in ['narrative', 'statcast', 'combo']:
        style_to_convert = args.commentary
        if style_to_convert == 'combo':
            style_to_convert = 'statcast' if args.terse else 'narrative'

        converter = GamedayConverter(
            game.gameday_data,
            style=style_to_convert,
            verbose_phrasing=not args.terse,
            use_bracketed_ui=args.bracketed_ui,
            commentary_seed=args.commentary_seed
        )
    ...
    # Gameday output is handled separately
    if args.commentary in ['gameday', 'combo']:
        ...
        # If no outfile, and it's the only commentary style, print to stdout
